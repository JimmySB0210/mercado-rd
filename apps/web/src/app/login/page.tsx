...
function LoginForm() {
    ...
    if (error) {
      setError(
        error.message === 'Invalid login credentials'
          ...
          : (typeof error.message === 'string' ? error.message : t('genericLoginError'))
      )
      setLoading(false)
      return
    }
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      setError(t('googleSignInErrorLogin'))
          ...
          <Link href="/" className="inline-block">
            <span className="text-3xl font-bold">
              {/* var(--color-blue-dark) — azul dedicado del logo (paleta oficial), no
                  var(--brand-blue) (ese sigue usándose en esta misma
                  página para el focus ring de los inputs) */}

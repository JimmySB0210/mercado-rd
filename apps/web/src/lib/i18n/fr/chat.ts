// ============================================================
// MercadoRD — i18n: namespace "chat" (français)
// Ruta: src/lib/i18n/fr/chat.ts
// ============================================================

import type { ChatDict } from '@/lib/i18n/es/chat'

export const chat = {
  loadingConversations: 'Chargement des conversations...',
  loadingChat: 'Chargement du chat...',
  defaultVendorName: 'Vendeur',
  defaultBuyerName: 'Acheteur',
  sendButton: 'Envoyer',
  messagePlaceholder: 'Écrivez un message...',

  messagesPageTitle: 'Messages',
  messagesPageSubtitle: 'Vos conversations avec les boutiques',
  asBuyerSectionTitle: "En tant qu'acheteur",
  asVendorSectionTitle: 'En tant que vendeur',
  emptyAsBuyer: "Vous n'avez encore aucun message en tant qu'acheteur",
  emptyAsVendor: "Vous n'avez encore aucun message en tant que vendeur",
  emptyGeneric: "Vous n'avez encore aucun message",
  noMessagesInConvoShort: 'Pas encore de messages',
  justNow: "à l'instant",
  minutesAgo: 'il y a {count} min',
  hoursAgo: 'il y a {count}h',
  daysAgo: 'il y a {count}j',

  backToMessagesLink: 'Retour aux messages',
  conversationNotFound: 'Cette conversation est introuvable.',
  noMessagesInThread: "Il n'y a pas encore de messages dans cette conversation.",
  verifiedTrustBadge: '✓ Vérifié',
  newOnMercadoRD: 'Nouveau sur MercadoRD',
  onMercadoRDSince: 'Sur MercadoRD {duration}',
  membershipMonthsSingular: 'il y a {count} mois',
  membershipMonthsPlural: 'il y a {count} mois',
  membershipYearsSingular: 'il y a {count} an',
  membershipYearsPlural: 'il y a {count} ans',

  attachFileAria: 'Joindre un fichier',
  removeAttachmentAria: 'Retirer la pièce jointe',
  viewAttachmentAria: 'Voir la pièce jointe',
  downloadAttachmentLabel: 'Télécharger',
  uploadingAttachmentsLabel: 'Envoi des pièces jointes...',
  attachmentUploadError: "Impossible d'envoyer une des pièces jointes. Réessayez.",

  onlineStatusLabel: '🟢 En ligne',
  offlineStatusLabel: '⚫ Hors ligne',

  requestQuoteButton: 'Demander un devis',
  quoteQuantityInputLabel: 'Quantité',
  sendQuoteRequestButton: 'Envoyer la demande',
  invalidQuantityError: 'Entrez une quantité valide',
  sendingButton: 'Envoi en cours...',
  cancelButton: 'Annuler',
  deliveryAddressLabel: "L'adresse de livraison",
  deliveryAddressPlaceholder: 'Rue, numéro, secteur, référence...',
  selectProvincePlaceholder: 'Sélectionnez une province',
  paymentCashLabel: 'Espèces',
  paymentTransferLabel: 'Virement',
  paymentAzulLabel: 'Carte (Azul)',
  paymentCardnetLabel: 'Carte (CardNet)',

  quoteCardTitle: 'Devis',
  quoteQuantityLine: 'Quantité : {count} unités',
  catalogPriceLine: 'Prix catalogue : {price}',
  waitingVendorResponse: 'En attente de la réponse du vendeur...',
  unitPriceInputLabel: 'Prix par unité (RD$)',
  sendQuoteButton: 'Envoyer',
  invalidPriceError: 'Entrez un prix valide',
  quoteActionError: 'Impossible de traiter le devis. Réessayez.',
  perUnitSuffix: '/ unité',
  subtotalLine: 'Sous-total : {amount}',
  acceptQuoteButton: 'Accepter le devis',
  confirmAcceptButton: 'Confirmer la commande',
  waitingBuyerAccept: "En attente de l'acceptation de l'acheteur...",
  adjustPriceLabel: 'Ajuster le prix',
  quoteAcceptedLabel: 'Devis accepté — commande créée',
  viewOrderLink: 'Voir la commande →',

  translateButton: 'Traduire',
  translatingButton: 'Traduction en cours...',
  hideTranslationButton: 'Masquer la traduction',
  translateError: 'Impossible de traduire le message. Réessayez.',

  recordVoiceMessageAria: 'Enregistrer un message vocal',
  micPermissionError: "Vous devez autoriser l'accès au microphone pour enregistrer un message vocal.",
  recordingLabel: 'Enregistrement',
  stopRecordingButton: 'Arrêter',
} satisfies ChatDict
